...
class JsonToSqlBuilder:
    ...
    def _build_select(self, tbl, select_items: List[Dict[str, Any]]):
        if not select_items:
    ...
    def _build_aggregation(self, tbl, aggregation: str, field: str):
        agg = aggregation.lower()

        if agg == "count":
            if field == "*":
                return func.count()
            return func.count(self._get_column(tbl, field))
        
        if agg == "count_distinct":
            if field == "*":
                raise QueryBuilderError("Agregação 'count_distinct' não pode ser usada com campo coringa '*'")
    ...
    def _build_filters(self, tbl, filters: List[Dict[str, Any]]):
        expressions = []
    ...
    def _get_column(self, tbl, field: str):
        if field == "*":
            raise QueryBuilderError("'*' só pode ser usado em algumas agregações, como count.")
        return tbl.c[field]

class SqlQueryBuilderComponent(Component):
    display_name = "SQL Query Builder"
    description = "Constroi uma consulta SQL a partir do json gerado pela LLM"
    ...
    def execute(self) -> Message:
        try:
            builder = JsonToSqlBuilder(database_url=self.database_url)
            stmt = builder.build(json.loads(self._clean_llm_json(self.query_plan)))
            result = str(stmt.compile(compile_kwargs={"literal_binds": True}))
            self.log("Code execution completed successfully")
            return Message(text=result)

        except QueryBuilderError as e:
            error_message = f"Error when building SQL query: {e!s}"
